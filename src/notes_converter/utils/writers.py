...
class DocxWriter:
    """A class containing all methods necessary to
    save to a Microsoft Word document.
    """
    ...
    def __init__(self, output_path, template_path=None) -> None:
        ...
        Returns
        -------
        An empty Word document in the given `output_path`.
        """
    ...
    def _create_empty_document(self):
        """Create an empty Word document at the location in `self.output_path`
        using the template located in `self.template_path`.
        """
        try:
    ...
    def write_notes(self, notes: List[Entry]):
        ...
        for note in notes:
            ...
            for value, body in enumerate(note.note_text.split("\n")):
